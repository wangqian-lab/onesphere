...
class OneshareMeasurementCalcItem(models.Model):
    ...
    def _get_measurement_item_domain(self):
        parent_quality_point_id = self.env.context.get(
            "default_parent_quality_point_id", False
        )
        if parent_quality_point_id:
            return [
                (
                    "measurement_item_id.parent_quality_point_id",
                    "=",
    ...
    measurement_calc_type_id = fields.Many2one(
        "oneshare.measurement.calc.type", required=True
    )

    parent_quality_point_id = fields.Many2one(
        "oneshare.quality.point", ondelete="cascade", index=True, string="Working Step"
    )

    note = fields.Char(string="Description")

    measurement_item_ids = fields.Many2many(
        "oneshare.measurement.item",
        "measurement_item_calc_type_rel",
        "calc_item_id",
        "measurement_item_id",
...
class OneshareMeasurementCalculateType(models.Model):
    ...
    _sql_constraints = [
        (
            "code_unique",
            "UNIQUE(code)",
            "Field code must be unique per measurement calculate type.",
        ),
    ]

    def name_get(self):
        return [
            (calc_type.id, "[{0}]{1}".format(calc_type.code, calc_type.desc))
            for calc_type in self
        ]
...
class OneshareMeasurementItem(models.Model):
    ...
    test_type = fields.Selection(
        [("passfail", "Pass - Fail"), ("measure", "Measure")],
        string="Test Type",
        default="passfail",
        required=True,
    )
