...
class MrpRoutingWorkcenter(models.Model):
    ...
    onesphere_bom_ids = fields.One2many(
        "onesphere.mrp.bom.operation.rel",
        "onesphere_operation_id",
    ...
    def button_open_mrp_workorder_step_action(self):
        ...
        action = self.env.ref("onesphere_core.onesphere_action_open_work_step").read()[
            0
        ]
        ctx = dict(
            self._context,
            default_operation_type="mrp_operation",
    ...
    # 重写删除，会将该作业与工步的关联也删掉
    def unlink(self):
        self.env["onesphere.mrp.operation.step.rel"].search(
