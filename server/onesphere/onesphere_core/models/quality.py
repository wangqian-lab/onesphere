...
def find_index_subdomain_include_name(domain=None, name="onesphere_operation_ids"):
    for i, subdomain in enumerate(domain):
        if (
            isinstance(subdomain, Collection)
            and len(subdomain) == 3
            and subdomain[0] == name
        ):
            return i
    return -1
...
class OneshareQuality(models.Model):
    ...
    def _compute_check_count(self):
        check_data = self.env["quality.check"].read_group(
            [("point_id", "in", self.ids)], ["point_id"], ["point_id"]
        )
        result = dict(
            (data["point_id"][0], data["point_id_count"]) for data in check_data
        )
        for point in self:
    ...
    can_do_skip = fields.Boolean(
        string="Allow Do Skip", default=False, help="Whether This Step Can Be Skipped"
    )
    can_do_rework = fields.Boolean(
        string="Allow Do Redo", default=True, help="Whether This Step Can Be Rework"
    )
    onesphere_operation_ids = fields.One2many(
        "onesphere.mrp.operation.step.rel",
        "work_step_id",
        string="Operations",
        check_company=True,
    )

    multi_measurement_ids = fields.One2many(
        "oneshare.measurement.item",
        "parent_quality_point_id",
        string="Measurement Items",
    )

    multi_measurement_calc_ids = fields.One2many(
        "oneshare.measurement.calc.item",
        "parent_quality_point_id",
        string="Measurement Calculate Items",
    )

    component_id = fields.Many2one(
        "product.product", "Product To Register", check_company=True
    )
    ...
    def default_get(self, fields_list):
        ...
        if operation_type:
            res.update({"operation_type_ids": [(4, operation_type.id)]})
        return res
    ...
    def search_panel_select_range(self, field_name, **kwargs):
        if field_name != "onesphere_operation_ids":
            ...
            return super(OneshareQuality, self).search_panel_select_range(
                field_name, **kwargs
            )
        steps = self.env["oneshare.quality.point"].search(
            [("onesphere_operation_ids", "=", False)]
        )
        steps = steps.ids
        field_range = {
            "0": {
        ...
        for operation in operations:
            bom_ids_list = (
                operation.onesphere_bom_ids.mapped("onesphere_bom_id").ids
                if operation.onesphere_bom_ids
                else [0]
            )
    ...
    def convert_subdomain_part(self, subdomain="", domain_part=2):
        ...
        work_step_domain = [
            ("onesphere_operation_ids.operation_id", "in", operation_ids.ids)
        ]
    ...
    def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None):
        ...
        return super(OneshareQuality, self).search_read(
