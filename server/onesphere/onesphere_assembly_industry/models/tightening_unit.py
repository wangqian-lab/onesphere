...
class OnesphereTighteningUnit(models.Model):
    ...
    tightening_controller_id = fields.Many2one(
        "maintenance.equipment",
        "Controller",
        ondelete="cascade",
        check_company=True,
        required=True,
        domain=[("technical_name", "in", ["tightening_controller"])],
    )
    category_id = fields.Many2one(
        "maintenance.equipment.category", related="tightening_controller_id.category_id"
    )
    serial_no = fields.Char(
        "Tightening Controller Serial Number",
        related="tightening_controller_id.serial_no",
    )

    company_id = fields.Many2one(
        "res.company", string="Company", related="tightening_controller_id.company_id"
    )
    partner_id = fields.Many2one(
        "res.partner",
        string="Vendor",
        related="tightening_controller_id.partner_id",
        check_company=True,
    )
    ...
    workcenter_id = fields.Many2one(
        "mrp.workcenter",
        string="Work Center",
        related="tightening_controller_id.workcenter_id",
        check_company=True,
        store=True,
    )
    ...
    def name_get(self):
        ...
        for unit in self:
            res.append(
                (
                    unit.id,
                    f"#{unit.ref}@{unit.serial_no}@{unit.workcenter_id.display_name}",
                )
            )
        return res
