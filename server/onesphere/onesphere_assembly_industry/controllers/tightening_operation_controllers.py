...
api_edit_tightening_opr_points_item = {
    ...
    "items": {
        "type": "object",
        "properties": {
            "sequence": {"type": "integer"},  # 序列
            "x_offset": {"type": "number"},  # x轴的位置
            "y_offset": {"type": "number"},  # y轴的位置
        },
        "required": ["sequence", "x_offset", "y_offset"],
    },
}
...
class OnesphereAssemablyIndustry(http.Controller):
    @http.api_route(
        "/api/v1/tightening_work_step/<int:tightening_work_step_id>/opr_points_edit",
        type="apijson",
        methods=["PUT", "OPTIONS"],
        auth="user",
        cors="*",
    ...
    def _edit_tightening_opr_points(self, tightening_work_step_id=None):
        ...
        tightening_work_step = env["oneshare.quality.point"].browse(
            tightening_work_step_id
        )

        body_tightening_points = request.ApiJsonRequest or []
        if not tightening_work_step:
            return oneshare_json_fail_response(
                msg="Tightening Work Step： %d not existed" % tightening_work_step_id,
            )
        ...
        for val in body_tightening_points:
            ...
            point_id = tightening_opr_point_obj.search(
                [
                    ("parent_quality_point_id", "=", tightening_work_step_id),
                    ("sequence", "=", sequence),
                ]
            ...
            if tightening_work_step.test_type == "promiscuous_tightening" and getattr(
                tightening_work_step, "tightening_tool_ids", None
            ):
                # 混杂默认默认取得原来的工步上定义的拧紧工具
                val.update(
                    {
                        "tightening_tool_ids": [
                            (6, 0, tightening_work_step.tightening_tool_ids.ids)
                        ]
                    }
                )
            val.update(
            ...
            if not point_id:
                ...
                val.update(
                    {
                        ...
                        "sequence": sequence,
                        "product_id": env.ref(
                            "onesphere_assembly_industry.product_product_screw_default"
                        ).id,  # 获取默认螺栓
                    }
                )
                ...
                if not ret:
                    _logger.error(
                        "Create Tightening Operation Point Fail, Vals: {0}".format(
                            pprint.pformat(val, indent=4)
                        )
                    )
            else:
                # 更新
                ...
                if not ret:
                    _logger.error(
                        "Update Tightening Operation Point Fail, vals: {0}".format(
                            pprint.pformat(val, indent=4)
                        )
                    )
