...
class UsersAPI(http.Controller):
    @http.api_route(
        "/api/v1/res.users", type="apijson", auth="none", cors="*", csrf=False
    )
    ...
    def pack_user_info(user_id):
        img_base64 = (
            user_id.partner_id.image_128.decode()
            if user_id.partner_id.image_128
            else ""
        )
        ret = {
            "id": user_id.id,
            ...
            "uuid": user_id.uuid,
            "hmi_role": user_id.hmi_role_id.code
            if user_id.hmi_role_id
            else "anonymous",
            "image_small": f"data:image/png;base64,{img_base64}",
        }
        if not user_id.active:
            ret.update({"hmi_role": "anonymous"})
        return ret

    @http.api_route(
        "/api/v1/res.users/<string:uuid>",
        type="apijson",
        auth="none",
        cors="*",
        csrf=False,
    ...
    @http.api_route(
        "/api/v1/res.users/login", type="apijson", auth="none", cors="*", csrf=False
    )
    def _login(self):
        ...
        if not request.session.db:
            return Response(
                json.dumps({"msg": "Database not found"}),
                headers={"content-type": "application/json"},
                status=HTTPStatus.BAD_REQUEST,
            )
        ...
        if not uid:
            return Response(
                json.dumps({"msg": "User Auth Fail"}),
                headers={"content-type": "application/json"},
                status=HTTPStatus.BAD_REQUEST,
            )

        user_id = (
            request.env["res.users"].sudo().with_context(active_test=False).browse(uid)
        )
    ...
    @http.api_route(
        "/api/v1/res.users/batch_archived",
        type="apijson",
        auth="none",
        cors="*",
        csrf=False,
    ...
    def _bach_patch_user_archived(self, **query_params):
        uuids = (
            query_params["uuids"].split(",") if query_params.get("uuids", "") else []
        )
        ...
        ret = user_ids.sudo().write({"active": False})
        if not ret:
            return Response(
                json.dumps({"msg": "Batch Archived fail"}),
                headers={"content-type": "application/json"},
